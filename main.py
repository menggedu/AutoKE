...
def main():
    ...
    parser.add_argument('--epoch-adam', default=10000, type=int,  help='epoch numbers for adam optimizer')
...
def train(args):
    ...
    # read config file 
    config_path = f'./config/{args.system}.json'
    if os.path.exists(config_path):
        with open(config_path, "r") as read_content:
            func_config = json.load(read_content)
    else:
        raise Exception(f"config file of {args.system} does not exist")
    ...
    model = PhysicsInformedNN(args, X_star,u_star, X_init, u_init, X_f_train, bc_lb, bc_ub, layers,
                                args.optimizer_name, args.lr, lb,ub,args.activation,
                                args.auto, 
